...
(function(){
  ...
  var waLink=function(){
    ...
    var lines=st.items.map(function(i){
      ...
      return '• '+t.name+(i.cfgLabel?' ('+fixCfg(i.cfgLabel)+')':'')+(i.qty>1?' × '+i.qty:'')+' — '+dd+' '+dayWord(dd)+
        (price==null?', цена по запросу':(i.unitPrice!=null?', '+fmt(price)+'/сутки':(t.priceNote?', '+t.priceNote+' ₸/сутки':'')));
    }).filter(Boolean);
    var msg='Здравствуйте! Хочу арендовать:\n'+lines.join('\n')+
      '\n\nДату выдачи согласуем'+
      (tt.net?'\nПримерно: '+(vary?'от ':'')+fmt(tt.net):'')+
